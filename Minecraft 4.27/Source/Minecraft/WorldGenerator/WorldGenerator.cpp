...
namespace WorldGenerator
{
    ...
    int ANOTHER_HEIGHT = SEA_HEIGHT + 5;
    std::map<float, float> ContinentalnessNodes = {
        {-1.0, ANOTHER_HEIGHT},
        {-0.5, ANOTHER_HEIGHT+5},
        {-0.45, ANOTHER_HEIGHT + 15},
        {0.5, ANOTHER_HEIGHT + 15},
        {0.65, ANOTHER_HEIGHT + 25},
        {0.7, ANOTHER_HEIGHT + 40},
        {0.8, ANOTHER_HEIGHT + 45},
    ...
    BlockEnumType getBlockType(float xCoor, float yCoor, int height, Biomes biomes, int z)
    {
        ...
        if (z > height)
        {
            return BlockEnumType::AIR;
        }
        if (z==height)
        {
            return BlockEnumType::DIRT;
        }
        return BlockEnumType::STONE;
    ...
    int getRealHeightAndBiomes(float x, float y, Biomes& biomes)
    {
        ...
        float view = preview.fractal(4,x,y);
        height = getSplineValue(view, ContinentalnessNodes);
        height += getSplineValue(ero, ErosionNodes);
        height += getSplineValue(pea, PeakValleysNodes);
